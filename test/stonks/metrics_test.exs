defmodule Stonks.MetricsTest do
  ...
  describe "twelvedata_requests" do
    ...
    test "create_twelvedata_request/1 with valid data creates a twelvedata_request" do
      valid_attrs = %{timestamp: ~T[14:00:00], url: "some url"}

      assert {:ok, %TwelvedataRequest{} = twelvedata_request} = Metrics.create_twelvedata_request(valid_attrs)
      assert twelvedata_request.timestamp == ~T[14:00:00]
    ...
    test "update_twelvedata_request/2 with valid data updates the twelvedata_request" do
      twelvedata_request = twelvedata_request_fixture()
      update_attrs = %{timestamp: ~T[15:01:01], url: "some updated url"}

      assert {:ok, %TwelvedataRequest{} = twelvedata_request} = Metrics.update_twelvedata_request(twelvedata_request, update_attrs)
      assert twelvedata_request.timestamp == ~T[15:01:01]
    ...
    test "update_twelvedata_request/2 with invalid data returns error changeset" do
      twelvedata_request = twelvedata_request_fixture()
      assert {:error, %Ecto.Changeset{}} = Metrics.update_twelvedata_request(twelvedata_request, @invalid_attrs)
    ...
    test "delete_twelvedata_request/1 deletes the twelvedata_request" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Metrics.get_twelvedata_request!(twelvedata_request.id) end
    end
